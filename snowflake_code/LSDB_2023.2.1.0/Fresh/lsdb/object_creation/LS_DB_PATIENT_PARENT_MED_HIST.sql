...
CREATE OR REPLACE PROCEDURE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.PRC_LS_DB_PATIENT_PARENT_MED_HIST()
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_PATIENT_PARENT_MED_HIST'),
	'LSDB','Case','LS_DB_PATIENT_PARENT_MED_HIST',null,:CURRENT_TS_VAR,null,null,null,null,null,'In Progress',null; 
...
select 'LS_DB_PATIENT_PARENT_MED_HIST' TARGET_TABLE_NAME,
nvl((SELECT LOAD_START_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL WHERE TARGET_TABLE_NAME='LS_DB_PATIENT_PARENT_MED_HIST' AND LOAD_STATUS = 'Completed' ORDER BY ROW_WID DESC limit 1),to_timestamp('1900-01-01','YYYY-MM-DD')) PARAM_VALUE,
'CDC_EXTRACT_TS_LB' PARAM_NAME; 




DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_PATIENT_PARENT_MED_HIST_DELETION_TMP;
CREATE TEMPORARY TABLE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_PATIENT_PARENT_MED_HIST_DELETION_TMP  As select RECORD_ID,'lsmv_parent_med_hist_episode' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_parent_med_hist_episode WHERE CDC_OPERATION_TYPE IN ('D') ;
DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST_TMP;
CREATE TEMPORARY TABLE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST_TMP  AS  WITH CD_WITH AS (select CD_ID,CD,DE,LN from 
(
...
,row_number() over(partition by LSMV_CODELIST_NAME.CODELIST_ID,LSMV_CODELIST_CODE.CODE,LSMV_CODELIST_DECODE.LANGUAGE_CODE 
                   order by LSMV_CODELIST_NAME.CDC_OPERATION_TIME  DESC,LSMV_CODELIST_CODE.CDC_OPERATION_TIME DESC,LSMV_CODELIST_DECODE.CDC_OPERATION_TIME DESC) rank
                                                                                      ...
                                                                                      (
                                                                                                     ...
                                                                                                     FROM ${stage_db_name}.${stage_schema_name}.LSMV_CODELIST_NAME WHERE CODELIST_ID IN ('1002','1017','158','9917')
...
select DISTINCT RECORD_ID record_id, 0 common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM ${stage_db_name}.${stage_schema_name}.lsmv_parent_med_hist_episode WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_PATIENT_PARENT_MED_HIST' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT 0 record_id, 0 common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM ${stage_db_name}.${stage_schema_name}.lsmv_parent_med_hist_episode WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_PATIENT_PARENT_MED_HIST' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
)
 ,
...
ON try_to_number(lsmv_parent_med_hist_episode_SUBSET.MEDICALEPISODENAME_CODE,38) =try_to_number(D_MEDDRA_ICD_SUBSET.LLT_CODE ,38)
and try_to_number(lsmv_parent_med_hist_episode_SUBSET.MEDICALEPISODENAME_PTCODE ,38) =try_to_number(D_MEDDRA_ICD_SUBSET.PT_CODE ,38)
...
SET REC_READ_CNT = (select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST_TMP)
...
'Case' ENTITY_NAME,
'lsmv_parent_med_hist_episode' SRC_TABLE_NAME, 
:CURRENT_TS_VAR LOAD_TS, 
'medicalepisodename_ptcode' ,
medicalepisodename_ptcode_1,
RECORD_ID,
CASE_NO,
'Expected Number,Received Character value on medicalepisodename_ptcode'
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST_TMP
WHERE (medicalepisodename_ptcode is null and medicalepisodename_ptcode_1 is not null)
...
medicalepisodename_code_1,
RECORD_ID,
CASE_NO,
'Expected Number,Received Character value on medicalepisodename_code'
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST_TMP
WHERE (medicalepisodename_code is null and medicalepisodename_code_1 is not null)
and ARI_REC_ID is not null 
and CASE_NO is not null;
...
INSERT INTO ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST
(
receipt_id    ,
case_no       ,
case_version  ,
...
ari_rec_id,
MEDICALEPISODENAME_PTCODE_MD_BK
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST_TMP TMP WHERE CONCAT(TMP.INTEGRATION_ID,'||',CASE WHEN 'YES'=(SELECT CHAR_PARAM_VALUE 
									...
									NOT IN (SELECT CONCAT(TGT.INTEGRATION_ID,'||',CASE WHEN 'YES'=(SELECT CHAR_PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='HISTORY_CONTROL' AND PARAM_NAME='KEEP_HISTORY') 
																				THEN TGT.PROCESSING_DT ELSE '9999-12-31' END) FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST TGT)
                                                                                ; 
COMMIT;
...
DELETE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST TGT
WHERE  ( record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_PATIENT_PARENT_MED_HIST_DELETION_TMP  WHERE TABLE_NAME='lsmv_parent_med_hist_episode')
)
--AND 'I' = (SELECT PARAM_NAME FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LOAD_CONTROL')
AND 'NO'=(SELECT CHAR_PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='HISTORY_CONTROL' AND PARAM_NAME='KEEP_HISTORY');


UPDATE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST TGT
...
REC_PROCESSED_CNT=(select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST where LOAD_TS= (select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST_TMP limit 1)),
LOAD_TS=(select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_PATIENT_PARENT_MED_HIST_TMP limit 1),
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_PATIENT_PARENT_MED_HIST'),
	'LSDB','Case','LS_DB_PATIENT_PARENT_MED_HIST',null,:CURRENT_TS_VAR,null,null,null,null,:line,'Error',null;
