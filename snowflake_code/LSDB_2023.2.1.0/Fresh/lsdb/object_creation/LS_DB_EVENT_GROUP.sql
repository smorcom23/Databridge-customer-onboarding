...
CREATE OR REPLACE PROCEDURE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.PRC_LS_DB_EVENT_GROUP()
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_EVENT_GROUP'),
	'LSDB','Case','LS_DB_EVENT_GROUP',null,:CURRENT_TS_VAR,null,null,null,null,null,'In Progress',null; 
...
select 'LS_DB_EVENT_GROUP' TARGET_TABLE_NAME,
nvl((SELECT LOAD_START_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL WHERE TARGET_TABLE_NAME='LS_DB_EVENT_GROUP' AND LOAD_STATUS = 'Completed' ORDER BY ROW_WID DESC limit 1),to_timestamp('1900-01-01','YYYY-MM-DD')) PARAM_VALUE,
'CDC_EXTRACT_TS_LB' PARAM_NAME; 




DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_EVENT_GROUP_DELETION_TMP;
CREATE TEMPORARY TABLE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_EVENT_GROUP_DELETION_TMP  As select RECORD_ID,'lsmv_event_group' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_event_group WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_event_group_search' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_event_group_search WHERE CDC_OPERATION_TYPE IN ('D') ;
DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP_TMP;
CREATE TEMPORARY TABLE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP_TMP  AS WITH CD_WITH AS (select CD_ID,CD,DE,LN from 
(
...
select DISTINCT RECORD_ID record_id, 0 common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_event_group_search WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_EVENT_GROUP' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT fk_event_grp_rec_id record_id, 0 common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_event_group_search WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_EVENT_GROUP' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT RECORD_ID record_id, 0 common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_event_group WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_EVENT_GROUP' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT 0 record_id, 0 common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_event_group WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_EVENT_GROUP' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
)
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_event_group_search
...
fk_event_grp_rec_id IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET))
 AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_EVENT_GROUP_DELETION_TMP  WHERE TABLE_NAME='lsmv_event_group_search')
  ) where REC_RANK=1 )
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_event_group
 WHERE  RECORD_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET) 
 AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_EVENT_GROUP_DELETION_TMP  WHERE TABLE_NAME='lsmv_event_group')
  ) where REC_RANK=1 )
...
SET REC_READ_CNT = (select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP_TMP)
...
UPDATE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP   
...
FROM 	${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP_TMP 
...
FROM 	${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP left join ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP_TMP 
...
and LS_DB_EVENT_GROUP.evntgrp_RECORD_ID in (select evntgrp_RECORD_ID from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP_TMP )
...
DELETE FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP TGT 
...
INSERT INTO ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP
( 
processing_dt ,
user_modified ,
date_modified ,
...
evntgrpsrch_date_created,
evntgrpsrch_condition
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP_TMP TMP where not exists (select 1 from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP TGT
...
DELETE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP TGT
WHERE  ( evntgrp_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_EVENT_GROUP_DELETION_TMP  WHERE TABLE_NAME='lsmv_event_group') OR evntgrpsrch_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_EVENT_GROUP_DELETION_TMP  WHERE TABLE_NAME='lsmv_event_group_search')
)
--AND 'I' = (SELECT PARAM_NAME FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LOAD_CONTROL')
AND 'NO'=(SELECT CHAR_PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='HISTORY_CONTROL' AND PARAM_NAME='KEEP_HISTORY');


UPDATE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP TGT
...
REC_PROCESSED_CNT=(select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP where LOAD_TS= (select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP_TMP limit 1)),
LOAD_TS=(select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_EVENT_GROUP_TMP limit 1),
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_EVENT_GROUP'),
	'LSDB','Case','LS_DB_EVENT_GROUP',null,:CURRENT_TS_VAR,null,null,null,null,:line,'Error',null;
