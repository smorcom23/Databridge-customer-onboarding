...
CREATE OR REPLACE PROCEDURE ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.PRC_LS_DB_MEDDRA_ICD()
...
insert into ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG (ROW_WID,TARGET_TABLE_NAME,PARAM_NAME)

select ROW_WID,TARGET_TABLE_NAME,PARAM_NAME from 
(select (select max( ROW_WID) from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG)+1 ROW_WID,'LS_DB_MEDDRA_ICD' AS TARGET_TABLE_NAME,'CDC_EXTRACT_TS_LB' PARAM_NAME
 union all select (select max( ROW_WID) from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG)+2,'LS_DB_MEDDRA_ICD','CDC_EXTRACT_TS_UB'
) where TARGET_TABLE_NAME not in (select TARGET_TABLE_NAME from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG)
;
...
SELECT (select nvl(max(row_wid)+1,1) from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_MEDDRA_ICD'),
...
SET PARAM_VALUE= nvl((SELECT LOAD_START_TS from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL WHERE TARGET_TABLE_NAME='LS_DB_MEDDRA_ICD'
...
drop table if exists ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_DELETION_TMP;
CREATE TEMPORARY TABLE ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_DELETION_TMP As 
select RECORD_ID,'LSMV_LOW_LEVEL_TERM' AS TABLE_NAME 
FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_LOW_LEVEL_TERM WHERE CDC_TRANSACTION_ID IN ('D')
UNION ALL
select RECORD_ID,'LSMV_MD_HIERARCHY' AS TABLE_NAME 
FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_MD_HIERARCHY WHERE CDC_TRANSACTION_ID IN ('D')
 ;



DROP TABLE if exists  ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_TMP;

CREATE  TABLE ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_TMP AS
WITH 
LSMV_CASE_NO_SUBSET as
 (
select DISTINCT RECORD_ID record_id, 'LSMV_LOW_LEVEL_TERM' AS TABLE_NAME,CAST(PT_CODE AS VARCHAR(250)) AS PT_CODE FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_LOW_LEVEL_TERM
	WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MEDDRA_ICD' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= (SELECT PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MEDDRA_ICD' AND PARAM_NAME='CDC_EXTRACT_TS_UB') 
UNION   
select DISTINCT RECORD_ID record_id, 'LSMV_MD_HIERARCHY' AS TABLE_NAME,CAST(PT_CODE AS VARCHAR(250)) AS PT_CODE  FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_MD_HIERARCHY WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MEDDRA_ICD' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= (SELECT PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MEDDRA_ICD' AND PARAM_NAME='CDC_EXTRACT_TS_UB')
...
,LLT_KANJI       
,LLT_NAME        
,CASE WHEN 'YES' = (SELECT PARAM_NAME FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='CHN') THEN LLT_NAME_CN ELSE CAST(NULL AS VARCHAR(500)) END AS LLT_NAME_CN
,LLT_WHOART_CODE 
...
FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_LOW_LEVEL_TERM 
WHERE( RECORD_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET where TABLE_NAME='LSMV_LOW_LEVEL_TERM') 
  ...
  RECORD_ID not in (select RECORD_ID from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_DELETION_TMP where TABLE_NAME='LSMV_LOW_LEVEL_TERM')
...
,HLGT_CODE     
,HLGT_NAME     
,CASE WHEN 'YES' = (SELECT CHAR_PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE PARAM_NAME='CHN') THEN hlgt_name_cn ELSE CAST(NULL AS VARCHAR(500)) END AS hlgt_name_cn
,HLT_CODE 
,HLT_NAME 
,CASE WHEN 'YES' = (SELECT CHAR_PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE PARAM_NAME='CHN') THEN hlt_name_cn ELSE CAST(NULL AS VARCHAR(500)) END AS hlt_name_cn
,replace(MEDDRA_VERSION,'v.','') AS mdhie_MEDDRA_VERSION
,NULL_FIELD 	
,PRIMARY_SOC_FG 
,CAST(PT_CODE AS VARCHAR(250)) AS PT_CODE  
,PT_NAME 
,CASE WHEN 'YES' = (SELECT CHAR_PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE PARAM_NAME='CHN') THEN pt_name_cn ELSE CAST(NULL AS VARCHAR(500)) END AS  pt_name_cn
,PT_SOC_CODE 
,RECORD_ID   
,SOC_ABBREV  
,SOC_CODE    
,SOC_NAME    
,CASE WHEN 'YES' = (SELECT CHAR_PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE PARAM_NAME='CHN') THEN soc_name_cn ELSE CAST(NULL AS VARCHAR(500)) END AS soc_name_cn
,SPR_ID 		
,USER_CREATED 	
,USER_MODIFIED  
...
,replace(MEDDRA_VERSION,'v.','') AS MEDDRA_VERSION  
,RANK() OVER ( PARTITION BY RECORD_ID ORDER BY CDC_OPERATION_TIME DESC ) REC_RANK
FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_MD_HIERARCHY
WHERE (RECORD_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET where TABLE_NAME='LSMV_MD_HIERARCHY' )
  or CAST(PT_CODE AS VARCHAR(250)) in (SELECT PT_CODE FROM LSMV_CASE_NO_SUBSET)
  )
  AND 
  RECORD_ID not in (select RECORD_ID from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_DELETION_TMP where TABLE_NAME='LSMV_MD_HIERARCHY')
...
select 
DISTINCT HLGT_CODE
,CASE WHEN 'YES' = (SELECT CHAR_PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE PARAM_NAME='CHN') THEN HLGT_NAME_CN ELSE HLGT_KANJI END   AS HLGT_NAME
,replace(MEDDRA_VERSION,'v.','') MEDDRA_VERSION
FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_HLGT_PREF_TERM
),
LSMV_PREF_TERM_SUBSET AS
(SELECT DISTINCT 
PT_CODE
,CASE WHEN 'YES' = (SELECT CHAR_PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE PARAM_NAME='CHN') THEN PT_NAME_CN ELSE PT_KANJI END   AS PT_NAME
,replace(MEDDRA_VERSION,'v.','') AS MEDDRA_VERSION
FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_PREF_TERM)
,
LSMV_HLT_PREF_TERM_SUBSET AS
(select DISTINCT 
HLT_CODE
,CASE WHEN 'YES' = (SELECT CHAR_PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE PARAM_NAME='CHN') THEN HLT_NAME_CN ELSE HLT_KANJI END AS HLT_NAME
,replace(MEDDRA_VERSION,'v.','') MEDDRA_VERSION
FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_HLT_PREF_TERM
),
LSMV_SOC_TERM_SUBSET AS
( 
SELECT DISTINCT 
SOC_CODE
,CASE WHEN 'YES' = (SELECT CHAR_PARAM_VALUE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE PARAM_NAME='CHN') THEN SOC_NAME_CN ELSE SOC_KANJI END   AS SOC_NAME
,replace(MEDDRA_VERSION,'v.','') MEDDRA_VERSION
FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.LSMV_SOC_TERM),
LSMV_SOC_ORDER_SUBSET AS
(SELECT DISTINCT
SOC_CODE
,SOC_ORDER
,INTERNATIONAL_SOC_ORDER
FROM ${stage_cntrl_db_name}.${stage_cntrl_schema_name}.ETL_SOC_ORDER
)
...
FROM
  ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD
...
SET REC_READ_CNT = (select count(LOAD_TS) from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_TMP)
where target_table_name='LS_DB_MEDDRA_ICD'
and LOAD_STATUS = 'In Progress'
and LOAD_START_TS=(select max(LOAD_START_TS) from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL 
						where target_table_name='LS_DB_MEDDRA_ICD'
					and LOAD_STATUS = 'In Progress') ;


INSERT INTO ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.TAB_DMIW(PT_CODE, ICD_CODE, LLT_CODE)
SELECT
  DISTINCT
   COALESCE(PT_CODE,'-1') PT_CODE, 
   --COALESCE(SOC_CODE,-1) SOC_CODE, 
   ...
   COALESCE(LLT_CODE,'-1') LLT_CODE
 FROM 
  ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_TMP
 WHERE
  BK_MEDDRA_ICD_WID=-1;
 
 
UPDATE ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_TMP 
SET BK_MEDDRA_ICD_WID= A.BK_MEDDRA_ICD_WID
FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.TAB_DMIW A
WHERE
...
UPDATE ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD   
...
FROM 	${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_TMP 
...
,MDHI_RECORD_ID
,LOWLE_RECORD_ID   
FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_TMP TMP 
where not exists (select 1 from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD TGT
...
DELETE FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD TGT
WHERE  ( LOWLE_RECORD_ID  in (SELECT RECORD_ID FROM  ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_DELETION_TMP  WHERE TABLE_NAME='LSMV_LOW_LEVEL_TERM') 
OR MDHI_RECORD_ID  in (SELECT RECORD_ID FROM  ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_DELETION_TMP  WHERE TABLE_NAME='LSMV_MD_HIERARCHY')
)
--AND 'I' = (SELECT PARAM_NAME FROM ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LOAD_CONTROL')
...
UPDATE  ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD TGT
...
REC_PROCESSED_CNT=(select count(LOAD_TS) from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD where LOAD_TS= (select LOAD_TS from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_TMP limit 1)),
LOAD_TS=(select LOAD_TS from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_MEDDRA_ICD_TMP limit 1),
LOAD_STATUS='Completed'
where target_table_name='LS_DB_MEDDRA_ICD'
and LOAD_STATUS = 'In Progress'
and LOAD_START_TS=(select max(LOAD_START_TS) from ${cntrl_transfm_db_name}.${cntrl_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_MEDDRA_ICD'
and LOAD_STATUS = 'In Progress') ;
