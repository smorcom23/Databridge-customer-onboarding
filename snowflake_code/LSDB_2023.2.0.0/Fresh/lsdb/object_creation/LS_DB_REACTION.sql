...
CREATE OR REPLACE PROCEDURE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.PRC_LS_DB_REACTION()
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_REACTION'),
                'LSDB','Case','LS_DB_REACTION',null,:CURRENT_TS_VAR,null,null,null,null,null,'In Progress',null; 
...
select 'LS_DB_REACTION' TARGET_TABLE_NAME,
nvl((SELECT LOAD_START_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL WHERE TARGET_TABLE_NAME='LS_DB_REACTION' AND LOAD_STATUS = 'Completed' ORDER BY ROW_WID DESC limit 1),to_timestamp('1900-01-01','YYYY-MM-DD')) PARAM_VALUE,
'CDC_EXTRACT_TS_LB' PARAM_NAME; 




DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_REACTION_DELETION_TMP;
CREATE TEMPORARY TABLE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_REACTION_DELETION_TMP  As select RECORD_ID,'lsmv_reaction' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_reaction WHERE CDC_OPERATION_TYPE IN ('D') ;
DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP;
CREATE TEMPORARY TABLE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP  AS  WITH CD_WITH AS (select CD_ID,CD,DE,LN from 
(
...
,row_number() over(partition by LSMV_CODELIST_NAME.CODELIST_ID,LSMV_CODELIST_CODE.CODE,LSMV_CODELIST_DECODE.LANGUAGE_CODE 
                   order by LSMV_CODELIST_NAME.CDC_OPERATION_TIME  DESC,LSMV_CODELIST_CODE.CDC_OPERATION_TIME DESC,LSMV_CODELIST_DECODE.CDC_OPERATION_TIME DESC) rank
                                                                                      ...
                                                                                      (
                                                                                                     ...
                                                                                                     FROM ${stage_db_name}.${stage_schema_name}.LSMV_CODELIST_NAME WHERE CODELIST_ID IN ('10002','10004','10005','1002','1002','1002','1002','1002','1002','1002','1002','1002','1002','1002','1002','1002','1002','1002','1002','10045','1011','1012','1012','10129','10130','10131','1015','1017','1017','1017','158','4','4','4','7077','7089','8106','8137','9065','9745','9864','9941','9941','9941','9941','9941','9941','9941','9941','9970')
...
select DISTINCT RECORD_ID record_id, 0 common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM ${stage_db_name}.${stage_schema_name}.lsmv_reaction WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_REACTION' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT 0 record_id, 0 common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM ${stage_db_name}.${stage_schema_name}.lsmv_reaction WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_REACTION' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
)
,
...
ON try_to_number(COALESCE(lsmv_reaction_SUBSET.REACTMEDDRALLT_CODE,lsmv_reaction_SUBSET.REACTMEDDRAPT_CODE),38)=try_to_number(D_MEDDRA_ICD_SUBSET.LLT_CODE,38) and try_to_number(lsmv_reaction_SUBSET.REACTMEDDRAPT_CODE,38)=try_to_number(D_MEDDRA_ICD_SUBSET.PT_CODE,38) 
and D_MEDDRA_ICD_SUBSET.PRIMARY_SOC_FG='Y'
LEFT JOIN D_MEDDRA_ICD_SUBSET As D_MEDDRA_ICD_SUBSET1
ON try_to_number(COALESCE(lsmv_reaction_SUBSET.REACTMEDDRALLT_CODE,lsmv_reaction_SUBSET.REACTMEDDRAPT_CODE),38)=try_to_number(D_MEDDRA_ICD_SUBSET1.LLT_CODE,38) and try_to_number(lsmv_reaction_SUBSET.REACTMEDDRAPT_CODE,38)=try_to_number(D_MEDDRA_ICD_SUBSET1.PT_CODE,38) 
 WHERE 1=1 
;
...
SET REC_READ_CNT = (select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP)
...
'Case' ENTITY_NAME,
'lsmv_reaction' SRC_TABLE_NAME, 
:CURRENT_TS_VAR LOAD_TS, 
'reactmeddrapt_code' ,
reactmeddrapt_code_1, 
RECORD_ID,
CASE_NO,
'Expected Number,Received Character value on reactmeddrapt_code'
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP
WHERE (reactmeddrapt_code is null and reactmeddrapt_code_1 is not null)
...
:CURRENT_TS_VAR LOAD_TS, 
'reactmeddrallt_code' ,
reactmeddrallt_code_1, 
RECORD_ID,
CASE_NO,
'Expected Number,Received Character value on reactmeddrallt_code'
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP
WHERE (reactmeddrallt_code is null and reactmeddrallt_code_1 is not null)
and ARI_REC_ID is not null 
and CASE_NO is not null;
...
RECORD_ID,
CASE_NO,
'Expected Number,Received Character value on reactduration'
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP
WHERE (reactduration is null and reactduration_1 is not null)
and ARI_REC_ID is not null 
and CASE_NO is not null;



DELETE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP
WHERE (reactduration is null and reactduration_1 is not null) or (reactmeddrallt_code is null and reactmeddrallt_code_1 is not null) or (reactmeddrapt_code is null and reactmeddrapt_code_1 is not null) 
and ARI_REC_ID is not null 
and CASE_NO is not null;
...
INSERT INTO ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION
(
receipt_id    ,
case_no       ,
case_version  ,
...
REACTMEDDRAPT_CODE_MD_BK1,
REACTMEDDRAPT_CODE_MD_BK
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP TMP WHERE CONCAT(TMP.INTEGRATION_ID,'||',CASE WHEN 'YES'=(SELECT CHAR_PARAM_VALUE 
                                                        ...
                                                        THEN TMP.PROCESSING_DT ELSE '9999-12-31' END ) 
                                                                                                                                                NOT IN (SELECT CONCAT(TGT.INTEGRATION_ID,'||',CASE WHEN 'YES'=(SELECT CHAR_PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='HISTORY_CONTROL' AND PARAM_NAME='KEEP_HISTORY') 
                                                                                                                                                                                                                                                                                                                                THEN TGT.PROCESSING_DT ELSE '9999-12-31' END) FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION TGT)
                                                                                ; 
COMMIT;
...
DELETE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION TGT
WHERE  ( record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_REACTION_DELETION_TMP  WHERE TABLE_NAME='lsmv_reaction')
)
--AND 'I' = (SELECT PARAM_NAME FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LOAD_CONTROL')
AND 'NO'=(SELECT CHAR_PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='HISTORY_CONTROL' AND PARAM_NAME='KEEP_HISTORY');


UPDATE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION TGT
...
REC_PROCESSED_CNT=(select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION where LOAD_TS= (select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP limit 1)),
LOAD_TS=(select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_REACTION_TMP limit 1),
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_REACTION'),
                'LSDB','Case','LS_DB_REACTION',null,:CURRENT_TS_VAR,null,null,null,null,:line,'Error',null;
