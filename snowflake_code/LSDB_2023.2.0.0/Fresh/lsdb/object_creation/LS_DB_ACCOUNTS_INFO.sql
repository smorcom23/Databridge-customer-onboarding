CREATE OR REPLACE PROCEDURE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.PRC_LS_DB_ACCOUNTS_INFO()
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_ACCOUNTS_INFO'),
                'LSDB','Case','LS_DB_ACCOUNTS_INFO',null,:CURRENT_TS_VAR,null,null,null,null,null,'In Progress',null; 
...
select 'LS_DB_ACCOUNTS_INFO' TARGET_TABLE_NAME,
nvl((SELECT LOAD_START_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL WHERE TARGET_TABLE_NAME='LS_DB_ACCOUNTS_INFO' AND LOAD_STATUS = 'Completed' ORDER BY ROW_WID DESC limit 1),to_timestamp('1900-01-01','YYYY-MM-DD')) PARAM_VALUE,
'CDC_EXTRACT_TS_LB' PARAM_NAME; 




DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP;
CREATE TEMPORARY TABLE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  As select RECORD_ID,'lsmv_account_accountgroup' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_accountgroup WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_account_company_unit' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_company_unit WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_account_prod_distribution' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_prod_distribution WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_account_product' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_product WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_accounts' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_accounts WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_accounts_contacts' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_accounts_contacts WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_accounts_group' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_accounts_group WHERE CDC_OPERATION_TYPE IN ('D') ;
DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO_TMP;
CREATE TEMPORARY TABLE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO_TMP  AS WITH CD_WITH AS (select CD_ID,CD,DE,LN from 
(
...
,row_number() over(partition by LSMV_CODELIST_NAME.CODELIST_ID,LSMV_CODELIST_CODE.CODE,LSMV_CODELIST_DECODE.LANGUAGE_CODE 
                   order by LSMV_CODELIST_NAME.CDC_OPERATION_TIME  DESC,LSMV_CODELIST_CODE.CDC_OPERATION_TIME DESC,LSMV_CODELIST_DECODE.CDC_OPERATION_TIME DESC) rank
                                                                                      ...
                                                                                      (
                                                                                                     ...
                                                                                                     FROM ${stage_db_name}.${stage_schema_name}.LSMV_CODELIST_NAME WHERE CODELIST_ID IN ('42','7032')
...
select DISTINCT RECORD_ID record_id, 0 common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_product WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT FK_ACCOUNT_PRODUCT_ID record_id, 0 common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_product WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT RECORD_ID record_id, 0 common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_accounts WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT 0 record_id, 0 common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_accounts WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT RECORD_ID record_id, 0 common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_accountgroup WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT fk_account_rec_id record_id, 0 common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_accountgroup WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT RECORD_ID record_id, 0 common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_accounts_contacts WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT fk_account_record_id record_id, 0 common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_accounts_contacts WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT RECORD_ID record_id, 0 common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_prod_distribution WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT FK_ACCOUNT_ID record_id, 0 common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_prod_distribution WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT RECORD_ID record_id, 0 common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_accounts_group WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT fk_account_record_id record_id, 0 common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_accounts_group WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT RECORD_ID record_id, 0 common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_company_unit WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT FK_ACCOUNT_RECORD_ID record_id, 0 common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_account_company_unit WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_ACCOUNTS_INFO' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
                AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
)
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_account_product
...
AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_account_product')
  ) where REC_RANK=1 )
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_accounts_group
WHERE ( RECORD_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET) OR
fk_account_record_id IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET))
AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_accounts_group')
  ) where REC_RANK=1 )
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_accounts_contacts
WHERE ( RECORD_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET) OR
fk_account_record_id IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET))
AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_accounts_contacts')
  ) where REC_RANK=1 )
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_account_accountgroup
WHERE ( RECORD_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET) OR
fk_account_rec_id IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET))
AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_account_accountgroup')
  ) where REC_RANK=1 )
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_account_prod_distribution
WHERE ( RECORD_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET) OR
FK_ACCOUNT_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET))
AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_account_prod_distribution')
  ) where REC_RANK=1 )
...
SET REC_READ_CNT = (select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO_TMP)
...
FROM    ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO_TMP 
...
FROM    ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO left join ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO_TMP 
...
and LS_DB_ACCOUNTS_INFO.acc_RECORD_ID in (select acc_RECORD_ID from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO_TMP )
...
DELETE FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO TGT 
...
INSERT INTO ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO
( 
processing_dt ,
user_modified ,
date_modified ,
...
accgrp_date_modified,
accgrp_date_created
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO_TMP TMP where not exists (select 1 from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO TGT
...
DELETE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO TGT
WHERE  ( accgrp_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_account_accountgroup') OR acccmpunit_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_account_company_unit') OR accprddist_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_account_prod_distribution') OR accprd_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_account_product') OR acc_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_accounts') OR acccont_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_accounts_contacts') OR acgrp_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_ACCOUNTS_INFO_DELETION_TMP  WHERE TABLE_NAME='lsmv_accounts_group')
)
...
UPDATE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO TGT
...
REC_PROCESSED_CNT=(select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO where LOAD_TS= (select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO_TMP limit 1)),
LOAD_TS=(select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ACCOUNTS_INFO_TMP limit 1),
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_ACCOUNTS_INFO'),
                'LSDB','Case','LS_DB_ACCOUNTS_INFO',null,:CURRENT_TS_VAR,null,null,null,null,:line,'Error',null;
