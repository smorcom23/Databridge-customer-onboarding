...
CREATE OR REPLACE PROCEDURE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.PRC_LS_DB_DEVICE_DEVICE_PROB_EVAL()
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_DEVICE_DEVICE_PROB_EVAL'),
	'LSDB','Case','LS_DB_DEVICE_DEVICE_PROB_EVAL',null,:CURRENT_TS_VAR,null,null,null,null,null,'In Progress',null; 
...
select 'LS_DB_DEVICE_DEVICE_PROB_EVAL' TARGET_TABLE_NAME,
nvl((SELECT LOAD_START_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL WHERE TARGET_TABLE_NAME='LS_DB_DEVICE_DEVICE_PROB_EVAL' AND LOAD_STATUS = 'Completed' ORDER BY ROW_WID DESC limit 1),to_timestamp('1900-01-01','YYYY-MM-DD')) PARAM_VALUE,
'CDC_EXTRACT_TS_LB' PARAM_NAME; 




DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DEVICE_DEVICE_PROB_EVAL_DELETION_TMP;
CREATE TEMPORARY TABLE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DEVICE_DEVICE_PROB_EVAL_DELETION_TMP  As select RECORD_ID,'lsmv_device_prob_eva' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_device_prob_eva WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_device_prob_eval_imdrf' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_device_prob_eval_imdrf WHERE CDC_OPERATION_TYPE IN ('D') ;
DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP;
CREATE TEMPORARY TABLE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP  AS WITH CD_WITH AS (select CD_ID,CD,DE,LN from 
(
...
,row_number() over(partition by LSMV_CODELIST_NAME.CODELIST_ID,LSMV_CODELIST_CODE.CODE,LSMV_CODELIST_DECODE.LANGUAGE_CODE 
                   order by LSMV_CODELIST_NAME.CDC_OPERATION_TIME  DESC,LSMV_CODELIST_CODE.CDC_OPERATION_TIME DESC,LSMV_CODELIST_DECODE.CDC_OPERATION_TIME DESC) rank
                                                                                      ...
                                                                                      (
                                                                                                     ...
                                                                                                     FROM ${stage_db_name}.${stage_schema_name}.LSMV_CODELIST_NAME WHERE CODELIST_ID IN ('9851','9945')
...
select DISTINCT record_id record_id, fk_device_rec_id common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM ${stage_db_name}.${stage_schema_name}.lsmv_device_prob_eval_imdrf WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_DEVICE_DEVICE_PROB_EVAL' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
...
select DISTINCT record_id record_id, fk_device_rec_id common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM ${stage_db_name}.${stage_schema_name}.lsmv_device_prob_eva WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_DEVICE_DEVICE_PROB_EVAL' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_device_prob_eval_imdrf
...
fk_device_rec_id IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET))
 AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DEVICE_DEVICE_PROB_EVAL_DELETION_TMP  WHERE TABLE_NAME='lsmv_device_prob_eval_imdrf')
  ) where REC_RANK=1 )
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_device_prob_eva
 WHERE ( record_id IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET) OR
fk_device_rec_id IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET))
 AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DEVICE_DEVICE_PROB_EVAL_DELETION_TMP  WHERE TABLE_NAME='lsmv_device_prob_eva')
  ) where REC_RANK=1 )
...
SET REC_READ_CNT = (select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP)
...
UPDATE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL   
...
FROM 	${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP 
...
FROM 	${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL left join ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP 
...
and LS_DB_DEVICE_DEVICE_PROB_EVAL.dvcprbeval_record_id in (select dvcprbeval_record_id from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP )
...
DELETE FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL TGT 
...
INSERT INTO ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL
( receipt_id    ,
case_no       ,
case_version  ,
processing_dt ,
...
dvcprbeval_date_created,
dvcprbeval_ari_rec_id
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP TMP where not exists (select 1 from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL TGT
...
DELETE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL TGT
WHERE  ( dvcprbeval_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DEVICE_DEVICE_PROB_EVAL_DELETION_TMP  WHERE TABLE_NAME='lsmv_device_prob_eva') OR dvcprbevalimdrf_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DEVICE_DEVICE_PROB_EVAL_DELETION_TMP  WHERE TABLE_NAME='lsmv_device_prob_eval_imdrf')
)
...
AND LS_DB_DEVICE_DEVICE_PROB_EVAL.INTEGRATION_ID = LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP.INTEGRATION_ID
AND LS_DB_DEVICE_DEVICE_PROB_EVAL.dvcprbeval_record_id = LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP.dvcprbeval_record_id
AND LS_DB_DEVICE_DEVICE_PROB_EVAL.EXPIRY_DATE = TO_DATE('9999-31-12','YYYY-DD-MM')
AND MD5(NVL(LS_DB_DEVICE_DEVICE_PROB_EVAL.dvcprbeval_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM')) ||'-'||NVL(LS_DB_DEVICE_DEVICE_PROB_EVAL.dvcprbevalimdrf_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM')) ) <> MD5(NVL(LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP.dvcprbeval_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM'))||'-'||NVL(LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP.dvcprbevalimdrf_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM')))
AND 'YES'=(SELECT CHAR_PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='HISTORY_CONTROL' AND PARAM_NAME='KEEP_HISTORY')	
;
...
UPDATE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL TGT
...
REC_PROCESSED_CNT=(select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL where LOAD_TS= (select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP limit 1)),
LOAD_TS=(select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DEVICE_DEVICE_PROB_EVAL_TMP limit 1),
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_DEVICE_DEVICE_PROB_EVAL'),
	'LSDB','Case','LS_DB_DEVICE_DEVICE_PROB_EVAL',null,:CURRENT_TS_VAR,null,null,null,null,:line,'Error',null;
