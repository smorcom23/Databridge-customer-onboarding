...
CREATE OR REPLACE PROCEDURE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.PRC_LS_DB_DLIST()
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_DLIST'),
	'LSDB','Case','LS_DB_DLIST',null,:CURRENT_TS_VAR,null,null,null,null,null,'In Progress',null; 
...
select 'LS_DB_DLIST' TARGET_TABLE_NAME,
nvl((SELECT LOAD_START_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL WHERE TARGET_TABLE_NAME='LS_DB_DLIST' AND LOAD_STATUS = 'Completed' ORDER BY ROW_WID DESC limit 1),to_timestamp('1900-01-01','YYYY-MM-DD')) PARAM_VALUE,
'CDC_EXTRACT_TS_LB' PARAM_NAME; 




DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DLIST_DELETION_TMP;
CREATE TEMPORARY TABLE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DLIST_DELETION_TMP  As select RECORD_ID,'lsmv_dlist_master' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_dlist_master WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_dlist_queue' AS TABLE_NAME FROM ${stage_db_name}.${stage_schema_name}.lsmv_dlist_queue WHERE CDC_OPERATION_TYPE IN ('D') ;
DROP TABLE IF EXISTS ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST_TMP;
CREATE TEMPORARY TABLE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST_TMP  AS WITH CD_WITH AS (select CD_ID,CD,DE,LN from 
(
...
select DISTINCT RECORD_ID record_id, fk_aim_rec_id common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_dlist_queue WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_DLIST' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT fk_aim_rec_id record_id, fk_aim_rec_id common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_dlist_queue WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_DLIST' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT RECORD_ID record_id, fk_aim_rec_id common_parent_key   FROM ${stage_db_name}.${stage_schema_name}.lsmv_dlist_master WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_DLIST' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
UNION 

select DISTINCT fk_aim_rec_id record_id, fk_aim_rec_id common_parent_key  FROM ${stage_db_name}.${stage_schema_name}.lsmv_dlist_master WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG_TMP WHERE TARGET_TABLE_NAME ='LS_DB_DLIST' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= :CURRENT_TS_VAR
)
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_dlist_queue
...
fk_aim_rec_id IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET))
 AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DLIST_DELETION_TMP  WHERE TABLE_NAME='lsmv_dlist_queue')
  ) where REC_RANK=1 )
...
FROM 
${stage_db_name}.${stage_schema_name}.lsmv_dlist_master
 WHERE ( RECORD_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET) OR
fk_aim_rec_id IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET))
 AND RECORD_ID not in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DLIST_DELETION_TMP  WHERE TABLE_NAME='lsmv_dlist_master')
  ) where REC_RANK=1 )
...
SET REC_READ_CNT = (select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST_TMP)
...
UPDATE ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST   
...
FROM 	${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST_TMP 
...
FROM 	${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST left join ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST_TMP 
...
and LS_DB_DLIST.dlistqueue_RECORD_ID in (select dlistqueue_RECORD_ID from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST_TMP )
...
DELETE FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST TGT 
...
INSERT INTO ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST
( 
processing_dt ,
user_modified ,
date_modified ,
...
dlistmaster_aer_version_no,
dlistmaster_aer_no
FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST_TMP TMP where not exists (select 1 from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST TGT
...
DELETE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST TGT
WHERE  ( dlistmaster_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DLIST_DELETION_TMP  WHERE TABLE_NAME='lsmv_dlist_master') OR dlistqueue_record_id  in (SELECT RECORD_ID FROM  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LSMV_DLIST_DELETION_TMP  WHERE TABLE_NAME='lsmv_dlist_queue')
)
...
AND LS_DB_DLIST.INTEGRATION_ID = LS_DB_DLIST_TMP.INTEGRATION_ID
AND LS_DB_DLIST.dlistqueue_RECORD_ID = LS_DB_DLIST_TMP.dlistqueue_RECORD_ID
AND LS_DB_DLIST.EXPIRY_DATE = TO_DATE('9999-31-12','YYYY-DD-MM')
AND MD5(NVL(LS_DB_DLIST.dlistmaster_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM')) ||'-'||NVL(LS_DB_DLIST.dlistqueue_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM')) ) <> MD5(NVL(LS_DB_DLIST_TMP.dlistmaster_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM'))||'-'||NVL(LS_DB_DLIST_TMP.dlistqueue_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM')))
AND 'YES'=(SELECT CHAR_PARAM_VALUE FROM ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='HISTORY_CONTROL' AND PARAM_NAME='KEEP_HISTORY')	
;
...
UPDATE  ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST TGT
...
REC_PROCESSED_CNT=(select count(LOAD_TS) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST where LOAD_TS= (select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST_TMP limit 1)),
LOAD_TS=(select LOAD_TS from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DLIST_TMP limit 1),
...
SELECT (select nvl(max(row_wid)+1,1) from ${tenant_transfm_db_name}.${tenant_transfm_schema_name}.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_DLIST'),
	'LSDB','Case','LS_DB_DLIST',null,:CURRENT_TS_VAR,null,null,null,null,:line,'Error',null;
