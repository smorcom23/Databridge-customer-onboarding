CREATE OR REPLACE PROCEDURE DATABRIDGE_DEV_DB.LSDB_TRANSFM.PRC_RECCNT_CHECK_STG_TRANSFM(run_date date)
...
v_run_date := COALESCE(run_date,DATEADD('DAY',-1,DATE(CURRENT_TIMESTAMP())));
...
(SCHEMA_LOCATION,TABLE_NAME_TRANSFM,JOIN_KEY_VALUE,PROCESSING_DT,RECORD_COUNTS )
                                        select *, count(*) from (select \'TRANSFM\',\'' || V_TABLE_DETAILS.TABLE_NAME_TRANSFM
                                        || '\',' || V_TABLE_DETAILS.RECCNT_KEY_NAME_TRANSFM
                                        || ',\'' || v_run_date
                                        || '\' from DATABRIDGE_DEV_DB.LSDB_TRANSFM.' 
                                        || V_TABLE_DETAILS.TABLE_NAME_TRANSFM || ' where date(\'' 
                                        || v_run_date || '\') <=  EXPIRY_DATE 
                                        QUALIFY ROW_NUMBER() OVER (PARTITION BY INTEGRATION_ID ORDER BY EXPIRY_DATE DESC) = 1 )
                                        group by 1,2,3,4 ;' ;
