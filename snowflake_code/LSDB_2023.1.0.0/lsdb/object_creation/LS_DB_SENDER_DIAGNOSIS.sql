...
CREATE OR REPLACE PROCEDURE $$TGT_DB_NAME.$$LSDB_TRANSFM.PRC_LS_DB_SENDER_DIAGNOSIS()
...
SELECT (select nvl(max(row_wid)+1,1) from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_SENDER_DIAGNOSIS'),
...
SET PARAM_VALUE= nvl((SELECT LOAD_START_TS from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_DATA_PROCESSING_DTL_TBL WHERE TARGET_TABLE_NAME='LS_DB_SENDER_DIAGNOSIS' AND LOAD_STATUS = 'Completed' ORDER BY ROW_WID DESC limit 1),to_timestamp('1900-01-01','YYYY-MM-DD'))
...
DROP TABLE IF EXISTS $$TGT_DB_NAME.$$LSDB_TRANSFM.LSMV_SENDER_DIAGNOSIS_DELETION_TMP;
CREATE TEMPORARY TABLE  $$TGT_DB_NAME.$$LSDB_TRANSFM.LSMV_SENDER_DIAGNOSIS_DELETION_TMP  As select RECORD_ID,'lsmv_sender_diagnosis' AS TABLE_NAME FROM $$STG_DB_NAME.$$LSDB_RPL.lsmv_sender_diagnosis WHERE CDC_OPERATION_TYPE IN ('D') ;
DROP TABLE IF EXISTS $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS_TMP;
CREATE TEMPORARY TABLE $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS_TMP  AS  WITH CD_WITH AS (select CD_ID,CD,DE,LN from 
(
...
select DISTINCT RECORD_ID record_id, 0 common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM $$STG_DB_NAME.$$LSDB_RPL.lsmv_sender_diagnosis WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_SENDER_DIAGNOSIS' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= (SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_SENDER_DIAGNOSIS' AND PARAM_NAME='CDC_EXTRACT_TS_UB')
UNION 

select DISTINCT 0 record_id, 0 common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM $$STG_DB_NAME.$$LSDB_RPL.lsmv_sender_diagnosis WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_SENDER_DIAGNOSIS' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= (SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_SENDER_DIAGNOSIS' AND PARAM_NAME='CDC_EXTRACT_TS_UB')
)
 ,
...
FROM 
$$STG_DB_NAME.$$LSDB_RPL.lsmv_sender_diagnosis
 WHERE  RECORD_ID IN (SELECT record_id FROM LSMV_CASE_NO_SUBSET) 
 AND RECORD_ID not in (SELECT RECORD_ID FROM  $$TGT_DB_NAME.$$LSDB_TRANSFM.LSMV_SENDER_DIAGNOSIS_DELETION_TMP  WHERE TABLE_NAME='lsmv_sender_diagnosis')
  ) where REC_RANK=1 )
   SELECT DISTINCT  to_date(GREATEST(
NVL(lsmv_sender_diagnosis_SUBSET.DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM'))
))
PROCESSING_DT 
,LSMV_COMMON_COLUMN_SUBSET.RECEIPT_ID ,LSMV_COMMON_COLUMN_SUBSET.CASE_NO ,LSMV_COMMON_COLUMN_SUBSET.AER_VERSION_NO  CASE_VERSION ,LSMV_COMMON_COLUMN_SUBSET.VERSION_NO,TO_DATE('9999-31-12','YYYY-DD-MM') EXPIRY_DATE	,lsmv_sender_diagnosis_SUBSET.USER_CREATED CREATED_BY,lsmv_sender_diagnosis_SUBSET.DATE_CREATED CREATED_DT,:CURRENT_TS_VAR LOAD_TS,lsmv_sender_diagnosis_SUBSET.user_modified  ,lsmv_sender_diagnosis_SUBSET.user_created  ,lsmv_sender_diagnosis_SUBSET.spr_id  ,lsmv_sender_diagnosis_SUBSET.senderdiagnosislevel  ,lsmv_sender_diagnosis_SUBSET.senderdiagnosis_coded_flag  ,lsmv_sender_diagnosis_SUBSET.record_id  ,lsmv_sender_diagnosis_SUBSET.pt_code  ,lsmv_sender_diagnosis_SUBSET.meddra_version  ,lsmv_sender_diagnosis_SUBSET.llt_decode  ,lsmv_sender_diagnosis_SUBSET.llt_code  ,lsmv_sender_diagnosis_SUBSET.inq_rec_id  ,lsmv_sender_diagnosis_SUBSET.fk_summary_rec_id  ,lsmv_sender_diagnosis_SUBSET.diagnosis_term  ,lsmv_sender_diagnosis_SUBSET.diagnosis_code  ,lsmv_sender_diagnosis_SUBSET.date_modified  ,lsmv_sender_diagnosis_SUBSET.date_created  ,lsmv_sender_diagnosis_SUBSET.comp_rec_id  ,lsmv_sender_diagnosis_SUBSET.coding_type  ,lsmv_sender_diagnosis_SUBSET.coding_comments  ,lsmv_sender_diagnosis_SUBSET.ari_rec_id ,CONCAT(NVL(lsmv_sender_diagnosis_SUBSET.RECORD_ID,-1)) INTEGRATION_ID FROM lsmv_sender_diagnosis_SUBSET  LEFT JOIN LSMV_COMMON_COLUMN_SUBSET ON lsmv_sender_diagnosis_SUBSET.RECORD_ID  =  LSMV_COMMON_COLUMN_SUBSET.record_id  WHERE 1=1  
;



UPDATE $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_DATA_PROCESSING_DTL_TBL
SET REC_READ_CNT = (select count(LOAD_TS) from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS_TMP)
where target_table_name='LS_DB_SENDER_DIAGNOSIS'
and LOAD_STATUS = 'In Progress'
and LOAD_START_TS=(select max(LOAD_START_TS) from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_SENDER_DIAGNOSIS'
					and LOAD_STATUS = 'In Progress') 
; 

UPDATE $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS   
...
FROM 	$$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS_TMP 
...
INSERT INTO $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS
(
receipt_id    ,
case_no       ,
case_version  ,
...
coding_comments,
ari_rec_id
FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS_TMP TMP WHERE CONCAT(TMP.INTEGRATION_ID,'||',CASE WHEN 'YES'=(SELECT CHAR_PARAM_VALUE 
									...
									NOT IN (SELECT CONCAT(TGT.INTEGRATION_ID,'||',CASE WHEN 'YES'=(SELECT CHAR_PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='HISTORY_CONTROL' AND PARAM_NAME='KEEP_HISTORY') 
																				THEN TGT.PROCESSING_DT ELSE '9999-12-31' END) FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS TGT)
                                                                                ; 
COMMIT;
...
DELETE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS TGT
WHERE  ( record_id  in (SELECT RECORD_ID FROM  $$TGT_DB_NAME.$$LSDB_TRANSFM.LSMV_SENDER_DIAGNOSIS_DELETION_TMP  WHERE TABLE_NAME='lsmv_sender_diagnosis')
)
--AND 'I' = (SELECT PARAM_NAME FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LOAD_CONTROL')
...
UPDATE  $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS TGT
...
REC_PROCESSED_CNT=(select count(LOAD_TS) from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS where LOAD_TS= (select LOAD_TS from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS_TMP limit 1)),
LOAD_TS=(select LOAD_TS from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_SENDER_DIAGNOSIS_TMP limit 1),
...
UPDATE $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_DATA_PROCESSING_DTL_TBL set ERROR_DETAILS=:line, LOAD_STATUS = 'Error' WHERE target_table_name='LS_DB_SENDER_DIAGNOSIS'
and LOAD_STATUS = 'In Progress'
;
