...
CREATE OR REPLACE PROCEDURE $$TGT_DB_NAME.$$LSDB_TRANSFM.PRC_LS_DB_MESSAGE_INFM_AUTH()
...
SELECT (select nvl(max(row_wid)+1,1) from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_DATA_PROCESSING_DTL_TBL where target_table_name='LS_DB_MESSAGE_INFM_AUTH'),
	'LSRA','Case','LS_DB_MESSAGE_INFM_AUTH',null,:CURRENT_TS_VAR,null,null,null,null,null,'In Progress',null;


UPDATE $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG
SET PARAM_VALUE= nvl((SELECT LOAD_START_TS from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_DATA_PROCESSING_DTL_TBL WHERE TARGET_TABLE_NAME='LS_DB_MESSAGE_INFM_AUTH' AND LOAD_STATUS = 'Completed' ORDER BY ROW_WID DESC limit 1),to_timestamp('1900-01-01','YYYY-MM-DD'))
...
DROP TABLE IF EXISTS $$TGT_DB_NAME.$$LSDB_TRANSFM.LSMV_MESSAGE_INFM_AUTH_DELETION_TMP;
CREATE TEMPORARY TABLE  $$TGT_DB_NAME.$$LSDB_TRANSFM.LSMV_MESSAGE_INFM_AUTH_DELETION_TMP  As select RECORD_ID,'lsmv_informed_authority' AS TABLE_NAME FROM $$STG_DB_NAME.$$LSDB_RPL.lsmv_informed_authority WHERE CDC_OPERATION_TYPE IN ('D') UNION ALL select RECORD_ID,'lsmv_st_message' AS TABLE_NAME FROM $$STG_DB_NAME.$$LSDB_RPL.lsmv_st_message WHERE CDC_OPERATION_TYPE IN ('D') ;
DROP TABLE IF EXISTS $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH_TMP;
CREATE TEMPORARY TABLE $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH_TMP  AS WITH CD_WITH AS (select CD_ID,CD,DE,LN from 
(
...
select DISTINCT RECORD_ID record_id, 0 common_parent_key,  (select record_id from (select  record_id, ROW_NUMBER() OVER (ORDER BY CDC_OPERATION_TIME DESC) RANK from LSMV_REPLICA.LSMV_RECEIPT_ITEM ri where RECEIPT_NO=ri.RECEIPT_NO) tmp where tmp.rank=1) ARI_REC_ID   FROM $$STG_DB_NAME.$$LSDB_RPL.lsmv_informed_authority WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MESSAGE_INFM_AUTH' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= (SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MESSAGE_INFM_AUTH' AND PARAM_NAME='CDC_EXTRACT_TS_UB')
UNION 

select DISTINCT FK_LSM_REC_ID record_id, 0 common_parent_key,  (select record_id from (select  record_id, ROW_NUMBER() OVER (ORDER BY CDC_OPERATION_TIME DESC) RANK from LSMV_REPLICA.LSMV_RECEIPT_ITEM ri where RECEIPT_NO=ri.RECEIPT_NO) tmp where tmp.rank=1) ARI_REC_ID   FROM $$STG_DB_NAME.$$LSDB_RPL.lsmv_informed_authority WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MESSAGE_INFM_AUTH' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= (SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MESSAGE_INFM_AUTH' AND PARAM_NAME='CDC_EXTRACT_TS_UB')
UNION 

select DISTINCT RECORD_ID record_id, 0 common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM $$STG_DB_NAME.$$LSDB_RPL.lsmv_st_message WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MESSAGE_INFM_AUTH' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= (SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MESSAGE_INFM_AUTH' AND PARAM_NAME='CDC_EXTRACT_TS_UB')
UNION 

select DISTINCT 0 record_id, 0 common_parent_key,  ARI_REC_ID ARI_REC_ID   FROM $$STG_DB_NAME.$$LSDB_RPL.lsmv_st_message WHERE CDC_OPERATION_TIME >(SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MESSAGE_INFM_AUTH' AND PARAM_NAME='CDC_EXTRACT_TS_LB')
	AND CDC_OPERATION_TIME<= (SELECT PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LS_DB_MESSAGE_INFM_AUTH' AND PARAM_NAME='CDC_EXTRACT_TS_UB')
)
 ,
...
FROM 	$$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH_TMP 
...
AND DECODE('YES',(SELECT CHAR_PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME='HISTORY_CONTROL'),
           ...
           AND MD5(NVL(LS_DB_MESSAGE_INFM_AUTH.stmsg_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM')) ||'-'||NVL(LS_DB_MESSAGE_INFM_AUTH.infmauth_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM')) ) <> MD5(NVL(LS_DB_MESSAGE_INFM_AUTH_TMP.stmsg_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM'))||'-'||NVL(LS_DB_MESSAGE_INFM_AUTH_TMP.infmauth_DATE_MODIFIED ,TO_DATE('1900-01-01','YYYY-DD-MM')))
...
FROM 	$$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH left join $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH_TMP 
...
and LS_DB_MESSAGE_INFM_AUTH.stmsg_RECORD_ID in (select stmsg_RECORD_ID from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH_TMP )
...
DELETE FROM  $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH TGT 
...
INSERT INTO $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH
( receipt_id    ,
case_no       ,
case_version  ,
processing_dt ,
...
infmauth_acknowledgement_number,
infmauth_ack_received
FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH_TMP TMP where not exists (select 1 from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH TGT
...
DELETE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH TGT
WHERE  ( infmauth_record_id  in (SELECT RECORD_ID FROM  $$TGT_DB_NAME.$$LSDB_TRANSFM.LSMV_MESSAGE_INFM_AUTH_DELETION_TMP  WHERE TABLE_NAME='lsmv_informed_authority') OR stmsg_record_id  in (SELECT RECORD_ID FROM  $$TGT_DB_NAME.$$LSDB_TRANSFM.LSMV_MESSAGE_INFM_AUTH_DELETION_TMP  WHERE TABLE_NAME='lsmv_st_message')
)
--AND 'I' = (SELECT PARAM_NAME FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='LOAD_CONTROL')
AND 'NO'=(SELECT CHAR_PARAM_VALUE FROM $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_ETL_CONFIG WHERE TARGET_TABLE_NAME ='HISTORY_CONTROL' AND PARAM_NAME='KEEP_HISTORY');
...
UPDATE  $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH TGT
...
REC_PROCESSED_CNT=(select count(LOAD_TS) from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH where LOAD_TS= (select LOAD_TS from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH_TMP limit 1)),
LOAD_TS=(select LOAD_TS from $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_MESSAGE_INFM_AUTH_TMP limit 1),
...
UPDATE $$TGT_DB_NAME.$$LSDB_TRANSFM.LS_DB_DATA_PROCESSING_DTL_TBL set ERROR_DETAILS=:line, LOAD_STATUS = 'Error' WHERE target_table_name='LS_DB_MESSAGE_INFM_AUTH'
and LOAD_STATUS = 'In Progress'
;
